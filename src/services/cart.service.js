...
const addProductToCart = async (user, productId, quantity) => {
  ...
  //   cartItems: { $elemMatch: { product: { $elemMatch: { _id: productId } } } },
  // });
  // if (findProductInCart) {
  //   throw new ApiError(httpStatus.BAD_REQUEST, "Product already in cart");
  // }

  // const findProductInDb = await Product.findById(productId);
  // if (!findProductInDb) {
  //   throw new ApiError(
  ...
  // const productAddToCart = await Cart.create({
  //   ...userCart,
  //   cartItems: [...cartItems, { product: findProduct, quantity: quantity }],
  // });
...
const checkout = async (user) => {
  ...
  console.log(defaultAddressFlag);

  // sub total of products
  let subTotal = [];
  subTotal = cart.cartItems.map((item) => {
  ...
  let currentBalance = user.walletMoney - totalSum;
  if (currentBalance < 0) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
